define(function () { 'use strict';
    ...
    var requestFactory = function requestFactory(url, options) {
      ...
      if (options.jwtTokenHandler) {
        flow = flow.then(options.jwtTokenHandler.getToken).then(function (token) {
          return {
      ...
      if (options.jwtTokenHandler) {
        flow = flow.then(function (response) {
          if (response.status === 401) {
            return options.jwtTokenHandler.refreshToken().then(options.jwtTokenHandler.getToken).then(function (token) {
      ...
      flow = flow.then(function (response) {
        ...
        if (response.errorCode) {
          ...
          err = new Error("".concat(responseCode, " : Request error"));
        }
