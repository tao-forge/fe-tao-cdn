define(['jquery', 'lodash', 'handlebars', 'core/encoder/encoders', 'core/filter/filters'], function ($, _, Handlebars, Encoders, Filters) { 'use strict';

    $ = $ && $.hasOwnProperty('default') ? $['default'] : $;
    _ = _ && _.hasOwnProperty('default') ? _['default'] : _;
    Handlebars = Handlebars && Handlebars.hasOwnProperty('default') ? Handlebars['default'] : Handlebars;
    Encoders = Encoders && Encoders.hasOwnProperty('default') ? Encoders['default'] : Encoders;
    Filters = Filters && Filters.hasOwnProperty('default') ? Filters['default'] : Filters;
    ...
    var resyncIndexes = function resyncIndexes(obj, path, $node) {
      ...
      if (_.isArray(values)) {
        _.forEach(values, function (value, position) {
          ...
          if ($node) {
            $node.children('[data-bind-index]').eq(position).attr('data-bind-index', position + '').data('bind-index', position + '');
          }
        });
      }
    };
    ...
    var _unbind = function _unbind($node, $container, eventName) {
      ...
      if ($node.length > 0) {
        ...
        if (bounds && _(bounds[eventName]).where({
    ...
    DataBinder.prototype._bindArrayNode = function _bindArrayNode($node, path, model, domFirst) {
      ...
      if (!$node.data('bound')) {
        values = locate(model, path); //the item content is either defined by an external template or as the node content

        if ($node.data('bind-tmpl')) {
          template = self.templates[$node.data('bind-tmpl')]; //fallback to inner template
        ...
        _.forEach(values, function (value, index) {
          ...
          self.bind($newNode, self.model, path + '.' + index + '.', domFirst); //listen for removal on the item node
    ...
    DataBinder.prototype._listenUpdates = function _listenUpdates($node, path, model) {
      ...
      _bindOnce($node, this.$container, 'change', function () {
        if ($node.is('[data-bind-each]')) {
          ...
          update(model, path, self._getNodeValue($node)); //if we remove an element of an array, we need to resync indexes and bindings
    ...
    DataBinder.prototype._listenAdds = function _listenAdds($node, path, model) {
      ...
      _bindOnce($node, this.$container, 'add', function (content, data) {
        ...
        self.$container.trigger('add.binder', [self.model]).trigger('change.binder', [self.model]); //rethrow on the node
      ...
      if ($node.is('[data-bind-index]')) {
        ...
        if ($parentNode.children('[data-bind-index]').length - 1 !== removedIndex) {
          ...
          $parentNode.children('[data-bind-index]').filter(':gt(' + removedIndex + ')').each(function () {
            var $item = $(this);
            var newIndex = parseInt($item.data('bind-index'), 10) - 1; //we also update the indexes
    ...
    DataBinder.prototype.bind = function bind($elt, model, prefix, domFirst) {
      ...
      prefix = prefix || '';
      domFirst = domFirst || false; //Array binding

      bindElements($elt, 'bind-each', '_bindArrayNode'); //Remove binding, if bound value === true, then path is removed from the model

      bindElements($elt, 'bind-rm', '_bindRmNode'); //simple binding (the container can also bound something in addition to children)
