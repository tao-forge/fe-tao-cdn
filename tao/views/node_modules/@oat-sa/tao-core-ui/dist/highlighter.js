  ...
  function highlighter (options) {
      ...
      ranges.forEach(function (range) {
        ...
        if (isRangeValid(range)) {
          ...
          if (isWrappable(range.commonAncestorContainer) && !isWrappingNode(range.commonAncestorContainer.parentNode)) {
            range.surroundContents(getWrapper(currentGroupId)); // now the fun stuff: highlighting a mix of text and DOM nodes
          } else {
            rangeInfos = {
              ...
              endNode: isElement(range.endContainer) ? range.endContainer.childNodes[range.endOffset - 1] : range.endContainer,
    ...
    function wrapTextNodesInRange(rootNode, rangeInfos) {
      ...
      for (i = 0; i < childNodes.length; i++) {
        ...
        currentNode = childNodes[i]; // split current node in case the wrapping start/ends on a partially selected text node
        ...
        if (isText(currentNode)) {
          wrapTextNode(currentNode, currentGroupId); // ... or continue deeper in the node tree
      ...
      for (i = 0; i < childNodes.length; i++) {
        ...
        if (isBlacklisted(currentNode)) {
          continue;
            ...
            textNodesIndex++; // an isolated node (= not followed by a highlightable text) with its whole content highlighted
